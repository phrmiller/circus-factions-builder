...
def convert_images():
    ...
    for ext in ["jpg", "jpeg", "png"]:
        image_files.extend(list(image_directory.glob(f"*.{ext}")))
...
def verify_yaml_data(file, file_data):
    ...
    if file_data.get("image") and not file_data.get("image-alt"):
        print(f"Error: {file.name}")
        print("Details: YAML has an image but no image-alt description.")
        sys.exit(1)
    if file_data["type"] == "short" and not (
        file_data.get("title") == file.name.rsplit(".", 1)[0] == file_data.get("date")
    ):
        print(f"Error: {file.name}")
        print("Details: For short posts the file name, title, and date must match.")
        sys.exit(1)
...
def gather_content_data(args):
    ...
    for file in posts_directory.glob("**/*.md"):
        try:
            ...
            file_data["description"] = typogrify(
                html.unescape(file_data["description"])
            )
            file_data["date"] = datetime.strptime(
    ...
    )  # Final object must be sorted by date
    (
...
def build_pages(content_data):
    remove_and_recreate_site_directory()  # Must come first! Don't make mistake again!
    ...
    for page in content_data:
        ...
        output_html = prettify_html(output_html)  # Nice but probably useless
...
def main():
    ...
    update_content_files()  # Must precede `gather` step. It changes content files!
    content_data = gather_content_data(
        args
    )  # Any validation or transformation happens here.
    gather_time = time.time()
    print(f"Gather: {"{:.0f}".format((gather_time - start_time) * 1000)} ms")

    build_pages(content_data)  # Annihilates the previous site directory.
    build_time = time.time()
    print(f"Build: {"{:.0f}".format((build_time - gather_time) * 1000)} ms")

    build_tailwind_css()  # Must happen after "build" in case Markdown has styles.
    tailwind_time = time.time()
    print(f"Tailwind: {"{:.0f}".format((tailwind_time - build_time) * 1000)} ms")
